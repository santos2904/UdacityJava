...
public class GradeBook {
    ...
    /**
     * Adds up all the scores for one student and returns the result. Only works
     * for a valid student index.
    ...
    /**
     * Calculates the totals for all the students and returns the results in a
     * completely filled array. The order of the totals matches the order of the
     * names in studentNames.
    ...
    /**
     * Calculates the column totals (the sum of of all students' scores for one
     * topic) for all topics in a completely filled array. The order of the
     * topcs matches the order of the topic names in topicNames.
    ...
    public double[] totalsForAllTopics() {
        ...
        double[] topicTotal = new double[numTopics];
        for (int i = 0; i < numTopics; i++) {
            for (int j = 0; j < numStudents; j++) {
                topicTotal[i] = topicTotal[i] + grades[j][i];
            }
        }
        return topicTotal;
    }
